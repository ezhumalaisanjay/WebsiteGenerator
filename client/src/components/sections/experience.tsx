...
export function ExperienceSection() {
  ...
  const { ref: timelineRef, isIntersecting: timelineVisible } = useIntersectionObserver({ threshold: 0.2 });
    ...
    <section id="experience" className="py-20 bg-white dark:bg-slate-800">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div ref={titleRef} className="text-center mb-16">
          <h2 className={`text-3xl sm:text-4xl font-bold text-slate-900 dark:text-white mb-4 ${titleVisible ? 'animate-rotateIn' : 'opacity-0'}`}>
        ...
        <div ref={timelineRef} className="relative">
          {/* Timeline Line */}
          <div className={`absolute left-1/2 transform -translate-x-1/2 w-1 h-full bg-slate-200 dark:bg-slate-700 hidden lg:block ${timelineVisible ? 'animate-slideInDown' : 'opacity-0'}`}></div>
            ...
            <div key={exp.id} className="relative mb-12 lg:mb-16">
              <div className="lg:flex lg:items-center">
                <div className="lg:w-1/2 lg:pr-8">
                  <Card className={`bg-slate-50 dark:bg-slate-700 p-6 shadow-lg lg:mr-8 transform hover:scale-105 transition-all duration-300 ${timelineVisible ? 'animate-fadeInLeft animate-stagger-1' : 'opacity-0'}`}>
                ...
                {/* Timeline Node */}
                <div className={`hidden lg:block absolute left-1/2 transform -translate-x-1/2 w-4 h-4 bg-primary-600 border-4 border-white dark:border-slate-800 rounded-full z-10 ${timelineVisible ? 'animate-scaleIn animate-stagger-2' : 'opacity-0'}`}></div>
                ...
                <div className="lg:w-1/2 lg:pl-8 mt-6 lg:mt-0">
                  <img
                    ...
                    className={`rounded-2xl shadow-lg w-full h-64 object-cover transform hover:scale-105 transition-all duration-300 ${timelineVisible ? 'animate-fadeInRight animate-stagger-3' : 'opacity-0'}`}
                  />
                </div>
              </div>
            </div>
